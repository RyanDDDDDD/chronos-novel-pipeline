...
def _load_doc() -> dict[str, Any]:
    ...
    return dict(get_world_repo().get() or {})


def _save_doc(doc: dict[str, Any]) -> tuple[bool, str]:
    from repositories import get_world_repo
...
def set_scalar_field(field: str, value: str) -> tuple[bool, str, dict[str, Any]]:
    if field not in WORLD_SCALAR_FIELDS:
        return False, f"未知标量字段「{field}」。", {}
    text = value.strip()
    if not text:
        return False, f"{_SCALAR_LABELS[field]} 不能为空。", {}
    doc = _load_doc()
    if _scalar_nonempty(doc, field):
        return False, f"{_SCALAR_LABELS[field]} 已有内容，请用 refine_world_{field} 修改。", {}
    doc[field] = text
    return True, "", doc


def refine_scalar_field(field: str, value: str) -> tuple[bool, str, dict[str, Any]]:
    if field not in WORLD_SCALAR_FIELDS:
        return False, f"未知标量字段「{field}」。", {}
    text = value.strip()
    if not text:
    ...
    if not _scalar_nonempty(doc, field):
        return False, f"{_SCALAR_LABELS[field]} 尚未构建，请先用 set_world_{field}。", {}
    doc[field] = text
    return True, "", doc


def add_list_entry(field: str, item: dict[str, Any]) -> tuple[bool, str, dict[str, Any]]:
    if field not in WORLD_LIST_FIELDS:
        return False, f"未知列表字段「{field}」。", {}
    ...
    if _find_entry_index(items, name) is not None:
        return False, f"「{_LIST_LABELS[field]}」中已存在「{name}」，请用 edit 修改或换名新增。", {}
...
def edit_list_entry(
    ...
    *,
    ...
    if not desc_text:
        return False, "desc 不能为空。", {}
    doc = _load_doc()
    items = _list_items(doc, field)
    idx = _find_entry_index(items, name)
    if idx is None:
        return False, f"「{_LIST_LABELS[field]}」中未找到「{name.strip()}」。", {}
...
def remove_list_entry(field: str, name: str) -> tuple[bool, str, dict[str, Any]]:
    if field not in WORLD_LIST_FIELDS:
        return False, f"未知列表字段「{field}」。", {}
    doc = _load_doc()
    items = _list_items(doc, field)
...
def persist_world_doc(doc: dict[str, Any], *, changed_field: str) -> tuple[bool, str]:
    ok, err = _save_doc(doc)
    if not ok:
        return False, err
