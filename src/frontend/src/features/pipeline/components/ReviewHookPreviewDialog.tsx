...
import { useReviewHookCard } from '@/features/pipeline/queries/reviewHookCard'
...
export default function ReviewHookPreviewDialog({
  name, title, onClose,
}: { name: string; title: string; onClose: () => void }) {
  const { data, isLoading } = useReviewHookCard(name, true)
    ...
    <Dialog open onOpenChange={(next) => { if (!next) onClose() }}>
      <DialogContent className="max-w-lg max-h-[80vh] flex flex-col overflow-hidden p-0">
        <DialogHeader className="px-6 py-4 border-b border-[var(--c-border-subtle)] shrink-0">
          <DialogTitle className="text-sm font-semibold">规则卡片预览</DialogTitle>
        ...
        <div className="px-6 py-4 overflow-y-auto text-sm text-[color:var(--c-text-secondary)]">
          ...
          {!isLoading && (!data || data.content == null) && (
            <p className="text-xs text-[color:var(--c-text-faint)]">该判官为纯代码判定，无规则卡片明文</p>
          )}
          {!isLoading && data?.content != null && (
