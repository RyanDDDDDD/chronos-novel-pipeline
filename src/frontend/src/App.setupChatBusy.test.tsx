...
vi.mock('@/shared/utils/novels', () => ({
  ...
  filterNovelsByName: (novels: { id: string }[]) => novels,
}))
...
function renderApp() {
  ...
  const store = buildTestStore({ setupChat: { busy: true, autoMode: false, pendingChoice: null } })
  return render(
...
describe('setup-chat 流式传输期间小说栏', () => {
  it('setupChatBusy 为 true 时小说栏切换按钮仍可点击', () => {
    renderApp()
