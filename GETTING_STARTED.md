...
| **Image generation** | WebUI "Service Config" page → Models → Image tab: add a custom entry (`llm.custom_models` with `provider: "image_gen"`), pick one — **Novita** (`service: "novita"`, checkpoint list fetched live) or **NovelAI** (`service: "novelai"`, paste a persistent API token + pick a fixed model tier; needs a NovelAI subscription). Then bind it as `model_ref` on the "Portrait Generation" node in the "Pipeline Config" view | Novita: saving the key triggers a model-list refresh; NovelAI: no refresh; binding takes effect on the next generation |
